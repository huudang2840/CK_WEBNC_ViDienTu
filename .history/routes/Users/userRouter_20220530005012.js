...
router.get("/forgot", checkLogin, (req, res) => {
  let type = req.flash("type");
  let error = req.flash("message");
  res.render("forgot-password", { error: error, type: type });
});

router.post("/forgot", checkLogin, async (req, res) => {
...
// Khôi phục mật khẩu
router.get("/reset/:token", checkLogin, async (req, res) => {
...
router.post("/reset/:token", checkLogin, async (req, res) => {
  let token = req.params.token;
