...
// POST Đăng ký
router.post("/register", validator, uploadMultiple, async (req, res) => {
  let result = validationResult(req);
  let { phone, name, email, address, birth } = req.body;
  let { back_IDcard, front_IDcard } = req.files;

  console.log(back_IDcard.path, front_IDcard.path);
  ...
  if (userPhone || userEmail) {
    ...
    if (result.errors.length === 0) {
      ...
      let user = new Account({
        ...
        address: address,
        birth: birth,
        front_IDcard: front_IDcard.path,
        back_IDcard: back_IDcard.path,
        username: username,
