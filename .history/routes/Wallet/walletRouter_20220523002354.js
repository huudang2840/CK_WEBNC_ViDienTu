...
router.get("/transfer", checkLogin, function (req, res, next) {
  res.render("wallet-transfer-phone", { title: "Wallet" });
});

router.post("/transfer", checkLogin, function (req, res, next) {
  let { phone } = req.body;
  console.log(phone);
  let value = findUserWithPhone(phone);
  
  let userReceive;
  try {
    userReceive = await userService;
  }
  catch (err)
