...
router.post("/recharge", checkLogin, async function (req, res, next) {
  ...
  let balance_after = wallet.wallet_balance + add_money;
  ...
  if (card) {
    await Wallet.findOneAndUpdate(
      { owner: username },
      { account_balance: balance_after, history: wallet_history }
    );
    await Card.findOneAndUpdate({ owner: username }, { number_card: number_card, CVV: CVV });
