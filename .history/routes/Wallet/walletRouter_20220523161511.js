...
router.post("/transfer-confirm", checkLogin, async function (req, res, next) {
  ...
  let walletUserCurrent = await Wallet.findOne({ owner: userReceive.username }).exec();
  ...
  if (person_pay === "me") {
    balance_after = Number(userCurrent.account_balance) - Number(fee) - Number(money_transfer);
    await Wallet.findOneAndUpdate(
