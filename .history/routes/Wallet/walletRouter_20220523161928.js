...
router.post("/transfer-confirm", checkLogin, async function (req, res, next) {
  ...
  if (person_pay === "me") {
    balance_after = Number(walletCurrent.account_balance) - Number(fee) - Number(money_transfer);
    ...
    await Wallet.findOneAndUpdate(
      ...
      { account_balance: Number(balance_after), history: wallet_history_current }
    );
  } else {
  }
