...
router.post("/phonecard", checkLogin, async function (req, res, next) {
  ...
  if (totalBill > walletCurrent.account_balance) {
    ...
    res.json({ message: "Mua thẻ thành công" });
  }
});
// Mua thẻ cào

// Xem lịch sử giao dịch
router.get("/history", checkLogin, function (req, res, next) {
  let walletCurrent = await Wallet.find({ owner: req.session.username}).exec();
  console.log(walletCurrent);
   res.render("wallet-history", { title: "Wallet" });
});
