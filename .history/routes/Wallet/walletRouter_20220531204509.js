...
router.post(
  ...
  async function (req, res, next) {
    ...
    // Kiểm tra xem có card và số tiền nạp có vượt quá hạn mức k
    if ((card && card.max_charge === undefined) || (card && card.max_charge >= add_money)) {
      // Số dư không đủ thực hiện
      ...
      // Các biến thay đổi sau khi nạp tiền
...
router.post(
  ...
  async function (req, res, next) {
    ...
    // Lấy thông tin của User nhận tiền
...
router.post(
  ...
  async function (req, res, next) {
    ...
    // Người gửi trả tiền và kiểm tra số dư
    if (
      person_pay === "me" &&
...
router.get("/history", checkLogin, checkNotUser, checkFirstLogin, async function (req, res, next) {
  ...
  // Danh sách lịch sử giao dịch sau khi sắp xếp theo thừ tự ngày tạo gần nhất
...
// Hàm mã điện thoại tự động
function randomPhoneCard() {
  return Math.floor(Math.random() * 90000) + 10000;
}

// Hàm mã id lịch sử giao dịch
function randomHistory() {
  return Math.floor(100000 + Math.random() * 900000);
}

// Hàm thông tin mail gửi Bill chyển tiền qua mail
