...
router.post("/transfer", checkLogin, async function (req, res, next) {
  ...
  let userCurrent = await findUser(req.session.username);

  let walletReceive = await Wallet.findOne({ owner: userReceive.username }).exec();
  let walletUserCurrent = await Wallet.findOne({ owner: userCurrent.username }).exec();
  console.log(walletReceive, walletUserCurrent);
  // res.json({ code: "OK" });
  res.render("wallet-transfer-cofirm");
});
