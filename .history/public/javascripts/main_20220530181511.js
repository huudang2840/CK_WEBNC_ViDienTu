...
displayFormatVND(accountBalance);

// accountBalance.innerHTML = formatVND(Number(accountBalance.innerHTML));
...
function displayFormatVND(balance) {
  let total;
  for (let i = 0; i < balance.length; i++) {
    let array = balance[i].innerHTML.split("");
    total = Number(array[0]);
  }
  balance.innerHTML = formatVND(total);
}
